...
import QuizImportEditor from "./QuizImportEditor";
...
export default async function Page({params}:{params:Promise<{id:string}>}){
 ...
 const {data:questions}=await supabase.from('quiz_questions').select('*').eq('course_id',id).order('order_no');
 ...
 return <AppShell role="admin" name={`${profile.first_name} ${profile.last_name}`}>
  ...
  <div style={{height:18}}/><QuizImportEditor courseId={id} questions={questions||[]} />
 </AppShell>
}
