...
export default async function AdminPage(){
  ...
  const {count:attemptCount}=await supabase.from("quiz_attempts").select("id",{count:"exact",head:true}).eq("status","submitted");
  return <AppShell role="admin" name={`${profile.first_name} ${profile.last_name}`}>
    ...
    <div className="kpi-grid">
      ...
      <div className="card kpi"><span>ทำข้อสอบแล้ว</span><b>{attemptCount??0}</b></div>
    </div>
    <div className="two-col">
      <section className="card panel">
        <div className="space"><div><div className="eyebrow">COURSES</div><h3>คอร์สทั้งหมด</h3></div></div>
        <div className="table-wrap"><table className="table"><thead><tr><th>Code</th><th>ชื่อคอร์ส</th><th>เกณฑ์</th><th>สถานะ</th><th></th></tr></thead><tbody>
          {(courses??[]).map((c:any)=><tr key={c.id}><td>{c.code}</td><td>{c.title}</td><td>{c.passing_progress}%</td><td>{c.is_published?<span className="badge good">Published</span>:<span className="badge warn">Draft</span>}</td><td><Link href={`/admin/courses/${c.id}`}>จัดการ →</Link></td></tr>)}
        </tbody></table></div>
